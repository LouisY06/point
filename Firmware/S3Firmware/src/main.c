...
static void handle_request(point_request_t *request, int64_t now) {
    ...
    switch (c.op) {
    case POINT_HELLO:
        reply[length++] = (point_sensors_present() ? 25 : 0) | (point_haptics_ready() ? 6 : 0);
        break;
    case POINT_HEADING:
    case POINT_ATTITUDE:
    ...
    case POINT_HAPTIC: {
        ...
        if (c.kind == 0 || (newer && fresh && after_stop)) {
            accepted = point_haptics_play(&c, now);
...
void app_main(void) {
    ...
    ESP_LOGI("point", "Ready: scan for Point S3. Serial h=180ms motor test; s=stop.");
}
