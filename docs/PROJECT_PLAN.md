...
September 20, build 27: live diagnostics showed system/gyro/accel/compass 3/0/3/3 (and 3/0/3/2) with the mounting map intact. Orientation readiness now accepts healthy fully calibrated fused orientation (system 3), even when the individual gyro offset score is lower. Startup still waits when neither gyro nor system is calibrated. Compass readiness, fresh quaternion, hardware health and lowered-hand gates remain enforced; the stronger demo pulse pattern remains unchanged.

September 20, build 28: transit walks now feed the same navigation controller and arrival rules as standalone walks. A single fix within 40 m or a GPS dropout within 150 m no longer ends walking guidance; two distinct fresh fixes within the normal 8 m arrival gate, or the existing “I’m at the stop” control, end the approach. The coordinator processes arrival after navigation finishes updating, avoiding reentrant state changes. Tests compare the actual queued haptic commands for bus/rail approaches and standalone walking, verify recovery after GPS loss, reject stale/duplicate arrivals, and resume pointing after alighting. Debug diagnostics now distinguish trip phase, connection and hardware-calibration status.

The same build addresses live post-reset levels 1/0/0/2: healthy fused orientation with system >0 is accepted while gyro offset calibration continues, retaining the compass ≥2, freshness, steady two-pose and hand-elevation gates. This is estimated usable orientation, not a claim of full calibration or measured heading accuracy; see [Adafruit’s calibration guidance](https://learn.adafruit.com/adafruit-bno055-absolute-orientation-sensor/device-calibration).

September 20, build 29: pointing setup now retains the completed downward pose across brief sensor, Bluetooth and sheet interruptions for the same glove within the existing two-minute window. Each measurement has a fixed direction and attempt ID, so a cancelled completion cannot advance a later capture. Completed pairs are atomically saved and read back before live publication and success feedback; older preference mappings migrate automatically. Save failures retain the measured result for retry. Capture failures are announced explicitly, and Done/swipe dismissal cannot silently cancel an active measurement. Diagnostics record capture outcomes and pending state. Regression tests cover interruptions, late completions, unstable/opposing poses, device changes, expiry, actual filesystem write failure, retry and restoration without preferences.
