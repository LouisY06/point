...
Pointing intent gate: automatic haptics now require a deliberate gesture in addition to the ±30° level band, because the back-of-hand IMU cannot tell an extended finger from a hand resting on a handlebar. Walking arms on a raise from hanging (below -45° to level within one second) and disarms half a second after the hand leaves the band; cycling arms on a held lift (≥40° up for 0.25 s) followed by level and closes itself after four seconds. Device setup has a Walking/Cycling choice; sustained GPS speed above 4 m/s for five seconds promotes walking to cycling and 2 m/s for five seconds restores it. The gate lives in `PointingIntentGate`, is applied by `FirmwareGlove` to outdoor, indoor strict and indoor relaxed pointing alike, and needs no firmware change. Thresholds are untested prototype values: the arm-swing false positive on foot and the lift gesture on a bike still need an on-device walk and ride with the orientation log open.
