...
3. Hold the glove still for a few seconds to settle the gyro. If its compass needs settling, move your hand gently away from magnets and metal, then hold still again. Six-face accelerometer calibration is not required. Down/up setup needs healthy fresh orientation with either gyro 3 or acquired fused orientation (system above 0). Compass readiness does not block these gravity poses. Navigation separately needs compass at least 2 and an acquired north reference (system above 0); accelerometer levels are informational.
...
The completed finger-axis mapping is saved locally on the iPhone, keyed to the glove’s Bluetooth identifier. It restores automatically when that same glove reconnects and negotiates orientation support, even before the gyro settles. Fresh, healthy orientation and either gyro 3 or system above 0 are still required for directional guidance. Disconnect, backgrounding, board reset and temporary sensor faults clear live readings but do not erase this saved mounting geometry. Compass settling still pauses navigation; saving the mount does not save or bypass the sensor’s current calibration levels. Previous app versions did not persist mappings, so one successful down/up setup in this version is needed. Use **Sensor moved · Set up again** whenever the sensor slips or is remounted; slipping cannot be detected automatically. Closing setup or losing a sensor reading cancels only the measurement in progress. A completed downward pose is kept for the same glove for up to two minutes, so the upward pose can be retried. Completed pairs are written atomically to Application Support/GlovePointing and read back before success is shown; previous preference-based mappings migrate automatically. If saving fails, both poses remain available for Save pointing setup without recapturing. Done and swipe dismissal are disabled during a measurement.
...
- **Outdoor pointing:** point within 25° of the estimated beacon direction for 200 ms to start confirmation pulses. Turning beyond 35°, lowering the hand, or losing valid sensor/location data stops them. Nearby GPS uncertainty no longer blocks vibration toward the estimate. Pointing keeps the last accepted location through GPS gaps up to 15 seconds; beacon arrival still requires fresh fixes. This is the default; no extra mode or setting is needed.
...
A saved finger-axis mapping is restored when the same Bluetooth glove negotiates orientation support, even before its gyro settles. The setup section then shows **Glove pointing · Saved** and hides the down/up capture steps. Temporary sensor readiness is reported separately. A gyro sub-score below 3 does not block orientation when the BNO055 reports acquired fused orientation (system above 0); compass, sensor health and freshness checks still apply. If neither gyro nor system is ready, guidance pauses without deleting the mounting map. Repeat the poses only after moving the sensor on the glove.

### Restart the hardware sensors

**Hardware sensor calibration → Restart sensor calibration** is separate from **Sensor moved · Set up again**. It stops motor output, restarts BNO055 fusion/calibration and remeasures the backup gyro bias. Keep the glove still until the gyro reaches 3/3 or fused orientation acquires its reference (system above 0), then move gently away from magnets if the compass needs settling. Full gyro offset calibration is not required to use the estimated fused orientation; it can continue improving in the background. The saved finger direction is preserved. A hardware reset changes the room direction reference, so restore it or place the demo beacon again. The control requires the firmware's opcode-6 capability; older firmware shows an update instruction. Serial `c` is an alternative. The DevKit addressable RGB LED follows motor pulses using the RMT driver. Its pin is GPIO48 on original DevKitC-1 boards or GPIO38 on revision 1.1; use the matching firmware environment. Earlier XIAO GPIO21 LED builds targeted the wrong board.
