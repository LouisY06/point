...
3. Hold the glove still for a few seconds to settle the gyro. If its compass needs settling, move your hand gently away from magnets and metal, then hold still again. Six-face accelerometer calibration is not required. Down/up setup needs healthy fresh orientation with either gyro 3 or fully calibrated fused orientation (system 3). Compass readiness does not block these gravity poses. Navigation separately needs compass at least 2 and an acquired north reference (system above 0); accelerometer levels are informational.
