...
This is separate from the sample outdoor route and from the new MBTA bus/train arrival test. Entering indoor demo ends any active walking/transit session, pending destination confirmation, speech capture, and route haptics. Placement uses AR tracking; pocket guidance uses experimental motion estimates; no GPS fix is required for room guidance. No place search, Apple Maps route, transit request, or geocoding is needed to enter or operate the indoor route.
...
3. For the first beacon, point your glove level toward the marker and tap **Place 1**. Keep pointing briefly while the app captures the room reference and places the beacon. There is no separate Align control. Add up to four beacons in visit order, then tap **Start**.
4. **Experimental pocket mode is on by default.** Face beacon 1, tap **Start pocket test**, and stay at that spot while putting the unlocked phone in a snug pocket. After eight seconds and one second of stillness, the app announces “ready”. Walk forward, turning your body around the table; keep the phone fixed in the pocket. Camera capture stops when you start. Lowering your glove stops haptics.
5. Distance is marked approximate. The phone counts acceleration-based steps and integrates gyro rotation about gravity; each step moves the estimate by the configured step length (default 0.65 m, adjustable under **⋯ → Demo settings**). No GPS or continuous camera tracking is used. Sideways/backward walking, phone movement in the pocket, false/missed steps, and gyro bias cause error. Sequential guidance is now the default. Estimated arrival requires a new step since the previous target and 0.6 seconds within an estimated 0.7 m radius; a spoken turn cue then directs you to the next beacon. This can trigger early or late because position is approximate. Enable **Point at any beacon** in settings for free pointing instead. **Next beacon** remains a manual override. Taking the phone out can corrupt the heading estimate, so restart placement after changing how it is carried.
6. To compare against camera tracking, turn off **Experimental pocket mode** before starting. Keep the camera uncovered. Only camera mode automatically advances after reaching within 35 cm horizontally for 0.5 seconds.
7. **Pause / Resume** controls guidance. **⋯ → Clear beacons** starts a new placement sequence. **Done** returns home.
...
**Experimental estimate, not measured position:** pocket mode snapshots all beacon coordinates and the last tracked camera position. It assumes the wearer is facing beacon 1 when the countdown ends; no movement is counted during pocketing. Gravity-projected user acceleration identifies alternating step peaks; the step length is a configurable assumption, not an independently measured distance. Gravity-projected phone gyro rotation updates body bearing, assuming forward walking and a fixed pocket. Phone direction never substitutes for glove pointing. No constant-acceleration double integration, frozen-position guidance, or invented confidence radius is used.
